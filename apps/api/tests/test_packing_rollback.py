"""CMP-015 rollback proofs: a failure partway through the packing
command's single write block must leave no partial packing event, input
line, finished-goods lot, or ledger debit behind — and the same Session
must remain usable afterward, and a reused command id retried with a
non-colliding payload after a rollback must still succeed exactly once."""
import uuid
...
def _build_scenario(db_session, tenant, user, farm, *, suffix=None):
    ...
    harvest = harvest_service.record_harvest(
        ...
        source_lines=[{"batch_carrier_assignment_id": assignment_ids[0], "harvested_weight_kg": Decimal("10.000"), "whole_unit_count": None, "note": None}],
    )
    lot_id = db_session.execute(
        text("SELECT id FROM harvested_produce_lots WHERE harvest_event_id = :eid"), {"eid": harvest.id}
    ).scalar_one()

    return {"batch": batch, "lot_id": lot_id}
...
def test_duplicate_finished_goods_lot_code_rollback_leaves_no_partial_rows(db_session, active_context_with_farm) -> None:
    ...
    event_count = db_session.execute(
        select(func.count()).select_from(PackingEvent).where(PackingEvent.tenant_id == tenant.id)
    ).scalar_one()
    assert event_count == 1, "the failed second packing command must not leave a second event behind"

    fg_count = db_session.execute(
        select(func.count()).select_from(FinishedGoodsLot).where(FinishedGoodsLot.tenant_id == tenant.id)
    ).scalar_one()
    assert fg_count == 1, "the failed second packing command must not leave a partial finished-goods lot behind"

    line_count = db_session.execute(
        select(func.count()).select_from(PackingInputLine).where(PackingInputLine.tenant_id == tenant.id)
    ).scalar_one()
    assert line_count == 1, "the failed second packing command must not leave a partial input line behind"

    debit_count = db_session.execute(
        select(func.count()).select_from(ProduceLotLedgerEntry)
        .where(ProduceLotLedgerEntry.produce_lot_id == lot_id, ProduceLotLedgerEntry.entry_kind == "packing_consumption")
    ).scalar_one()
    assert debit_count == 1, "the failed second packing command must not leave a partial/orphan debit behind"

    # Existing harvest_receipt untouched.
    receipt_count = db_session.execute(
        select(func.count()).select_from(ProduceLotLedgerEntry)
        .where(ProduceLotLedgerEntry.produce_lot_id == lot_id, ProduceLotLedgerEntry.entry_kind == "harvest_receipt")
    ).scalar_one()
    assert receipt_count == 1


@pytest.mark.integration
def test_reused_command_id_after_rollback_creates_exactly_one_debit(db_session, active_context_with_farm) -> None:
    tenant, user, _headers, farm = active_context_with_farm
    suffix = uuid.uuid4().hex[:6]
    s = _build_scenario(db_session, tenant, user, farm, suffix=suffix)
    lot_id = s["lot_id"]
    ...
    event = packing_service.record_packing(
        db_session, tenant_id=tenant.id, farm_id=farm.id, actor_user_id=user.id, client_command_id=command_id,
        effective_time=_now(), finished_goods_lot_code=f"FRESH-{suffix}", package_count=1,
        packed_output_weight_kg=Decimal("2.000"), process_loss_weight_kg=Decimal("0"), rejected_weight_kg=Decimal("0"),
        note=None,
        input_lines=[{"harvested_produce_lot_id": lot_id, "consumed_weight_kg": Decimal("2.000"), "consumed_whole_unit_count": None, "note": None}],
    )
    debit_count = db_session.execute(
        select(func.count()).select_from(ProduceLotLedgerEntry).where(ProduceLotLedgerEntry.packing_event_id == event.id)
    ).scalar_one()
