"""CMP-013 rollback proofs: a failure partway through the single write block
must leave no partial writes — no harvest event, no source line, no produce
lot, no audit event — and the Session must remain usable afterward."""
import uuid
...
def test_duplicate_lot_code_rolls_back_completely(db_session, active_context_with_farm) -> None:
    ...
    line_count_for_second_assignment = db_session.execute(
        select(func.count()).select_from(HarvestSourceLine).where(
            HarvestSourceLine.batch_carrier_assignment_id == aids[1]
        )
    ).scalar_one()
    assert line_count_for_second_assignment == 0, "the failed harvest must not leave a source line behind"

    lot_count = db_session.execute(
        select(func.count()).select_from(HarvestedProduceLot).where(HarvestedProduceLot.batch_id == s["batch"].id)
    ).scalar_one()
    assert lot_count == 1, "the failed harvest must not leave a second produce lot behind"
...
def test_reused_command_id_after_rollback_is_a_genuine_new_attempt(db_session, active_context_with_farm) -> None:
    ...
    assert event.client_command_id == command_id
