"""NURSERY-OPS-005B downgrade-guard proof tests for migration
1ffda251c3a8, mirroring `test_intersalads_transplant_downgrade_guard.py`'s
established pattern exactly for the sibling
production_cultivation_plate -> grow_table compatibility rule: commit real
rows on a dedicated connection, attempt a downgrade past the migration,
confirm the guard blocks it, then clean up via
...
_PRE_MIGRATION_REVISION = "283bad02bb69"
...
def _create_minimal_tenant(session, *, code_suffix: str):
    ...
    tenant = tenant_service.create_tenant(session, code=f"lpguard-{code_suffix}", name="Leafy Production Guard Tenant")
    user = user_service.create_user(
        session, oidc_issuer="lpguard", oidc_subject=code_suffix, email=f"lpguard-{code_suffix}@example.com",
        display_name="Leafy Production Guard User",
    )
...
def _place_plate_on_grow_table(session, *, tenant, user, farm):
                        ...
                        spans=[
                            SpanSetupConfig(
                                ...
                                tables=TableGeneratorConfig(
                                    code_prefix=f"T{suffix[:4]}", start=1, end=1, pad_width=2, capacity=4,
                                ),
                            )
                        ],
                    )
    ...
    table_id = structure.leafy_zones[0].spans[0].tables[0].id
...
def test_downgrade_blocked_when_plate_occupies_grow_table(test_engine, alembic_head_restore) -> None:
    ...
    try:
        ...
        _place_plate_on_grow_table(session, tenant=tenant, user=user, farm=farm)
        session.commit()

        with pytest.raises(RuntimeError, match="currently occupy a Leafy grow_table"):
...
def test_downgrade_clean_when_no_live_occupancy_exists(test_engine, alembic_head_restore) -> None:
    ...
    with test_engine.connect() as verify_conn:
        ...
        compat_gone = verify_conn.execute(
            text(
                ...
                "JOIN carrier_types ct ON ct.id = r.occupant_carrier_type_id AND ct.code = 'production_cultivation_plate' "
                "JOIN location_types lt ON lt.id = r.target_location_type_id AND lt.code = 'grow_table'"
            )
        ).scalar_one()
        # The pre-existing generic and sibling rules must remain untouched.
        legacy_generic_intact = verify_conn.execute(
            text(
                "SELECT count(*) FROM occupancy_compatibility_rules r "
                "JOIN carrier_types ct ON ct.id = r.occupant_carrier_type_id AND ct.code = 'cultivation_plate' "
                "JOIN location_types lt ON lt.id = r.target_location_type_id AND lt.code = 'grow_table'"
            )
        ).scalar_one()
        sibling_intact = verify_conn.execute(
            text(
                "SELECT count(*) FROM occupancy_compatibility_rules r "
    ...
    assert legacy_generic_intact == 1
    assert sibling_intact == 1
    command.upgrade(_cfg(), "head")
    _assert_at_head(test_engine)
    with test_engine.connect() as verify_conn2:
        compat_restored = verify_conn2.execute(
            text(
                "SELECT count(*) FROM occupancy_compatibility_rules r "
                "JOIN carrier_types ct ON ct.id = r.occupant_carrier_type_id AND ct.code = 'production_cultivation_plate' "
                "JOIN location_types lt ON lt.id = r.target_location_type_id AND lt.code = 'grow_table'"
