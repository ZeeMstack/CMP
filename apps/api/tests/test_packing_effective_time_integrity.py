        ...
        with pytest.raises(Exception, match="cannot be in the future"):
            conn.execute(
                text(
                    ...
                    "SELECT gen_random_uuid(), tenant_id, farm_id, crop_id, variety_id, :specid, 1.000, 1.000, 0, 0, "
                ...
                {"uid": scenario["user_id"], "lid": scenario["lot_a_id"],
                 "specid": scenario["pack_specification_version_id"]},
            )
    finally:
        trans.rollback()
        conn.close()
...
def test_packing_input_line_before_source_lot_effective_time_rejected(test_engine) -> None:
    ...
    try:
        ...
        with pytest.raises(Exception, match="precedes source graded produce lot"):
            conn.execute(
                text(
...
def test_packing_input_line_before_latest_ledger_entry_rejected(test_engine) -> None:
    """GPL A's ledger already has a grading_receipt at T_grade, and (after
    a real packing consumption below) a packing_consumption at T_pack >
    T_grade. A second, raw packing_event whose effective_time sits
    between T_grade and T_pack must be rejected when it tries to
    reference GPL A again — not because it precedes the lot's own
    ...
    with test_engine.connect() as read_conn:
        lot_effective_time = read_conn.execute(
            text("SELECT effective_time FROM graded_produce_lots WHERE id = :lid"), {"lid": scenario["gpl_a_id"]}
        ).scalar_one()
    ...
    packing_service.record_packing(
        ...
        pack_specification_version_id=scenario["pack_specification_version_id"], effective_time=later_time,
    ...
    try:
        ...
        with pytest.raises(Exception, match="precedes the latest existing ledger entry for source graded produce lot"):
            conn.execute(
                text(
                    "INSERT INTO packing_input_lines "
                    "(id, tenant_id, farm_id, packing_event_id, graded_produce_lot_id, consumed_weight_kg, "
...
def test_packing_input_line_equal_to_lot_effective_time_accepted(test_engine) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            {"tid": scenario["tenant_id"], "fid": scenario["farm_id"], "eid": event_id, "lid": scenario["gpl_a_id"]},
        )
    finally:
        trans.rollback()
        conn.close()
