...
def _validate_hierarchy(db: Session, *, parent_type_id: uuid.UUID | None, child_type_id: uuid.UUID) -> None:
    if parent_type_id is None:
        condition = LocationTypeHierarchyRule.parent_type_id.is_(None)
    else:
        condition = LocationTypeHierarchyRule.parent_type_id == parent_type_id
    exists = db.execute(
        select(LocationTypeHierarchyRule.id).where(
            condition, LocationTypeHierarchyRule.child_type_id == child_type_id
        )
    ).scalar_one_or_none()
    if exists is None:
        raise InvalidLocationHierarchyError(f"{parent_type_id}:{child_type_id}")
...
def create_location(
    ...
    *,
    ...
    _validate_hierarchy(db, parent_type_id=parent_type_id, child_type_id=location_type.id)
...
def bulk_generate_children(
    ...
    *,
    ...
    location_type = _get_location_type_by_code(db, location_type_code)
    _validate_hierarchy(db, parent_type_id=parent.location_type_id, child_type_id=location_type.id)
