...
/** NURSERY-OPS-002A: Germination Placement -- physical placement only (a
 * Germination Trolley Asset occupies a Germination Chamber Location
 * directly; a Seed Tray Carrier occupies a Trolley Slot). No biological
 * Germination outcome field belongs in either form. Mirrors nursery.ts's
...
export const placeTrayFormSchema = z.object({
  ...
  trolley_id: z.string().min(1, "Trolley is required"),
  slot_id: z.string().min(1, "Slot is required"),
  effective_date: z.string().min(1, "Date is required"),
  effective_time_of_day: z.string().min(1, "Time is required"),
  reason: z.string(),
});
