  ...
  return (
    <li className="flex flex-col gap-2 rounded-xl border border-border-subtle bg-surface p-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-col gap-1">
        ...
        {/* Original Packing receipt -- immutable, distinct from the live
            balance lines below. */}
        <span className="text-xs text-ink-muted">
        ...
        {placementQuery.data ? (
          <span className="text-xs text-ink-muted">
            <span className="font-semibold text-ink">
              Available {placementQuery.data.available_weight_kg} kg / {placementQuery.data.available_package_count} pkg
            </span>
            {" — Placed "}
            {placementQuery.data.total_placed_weight_kg} kg{" — Unplaced "}
            {placementQuery.data.unplaced_weight_kg} kg
          </span>
        ) : (
          <span className="text-xs text-ink-muted">
            {placementQuery.isLoading ? "Loading placement…" : "Placement unavailable"}
          </span>
        )}
