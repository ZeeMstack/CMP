...
It may reference (never duplicate) the existing Observation architecture: `record_inspection` optionally calls `observation_service.record_observation_in_transaction` (the non-committing, transaction-composable form of `record_observation`) in the same command with the same resolved `effective_time`, storing the resulting `observation_event_id` — one `ObservationEvent` can hold many typed `ObservationValue`s, so "GrowerInspection -> links to ObservationEvents" is satisfied by this single reference rather than a second value-storage table.
...
**Atomicity (UX-OPS-001C/R1, N05).** `record_inspection` owns ONE transaction and ONE commit covering the `ObservationEvent`, its `ObservationValue`s, the `GrowerInspection`, its `InspectionFinding`s, and both audit events (`crop_batch.observation_recorded`, `grower_inspection.recorded`). Any failure rolls the whole command back — never an orphaned `ObservationEvent`. The standalone Observation command (`record_observation`, used by the Observation API and Germination outcome recording) is unchanged: it wraps the same composable write and commits on its own. Proven by `tests/test_grower_inspection_atomicity.py`.
