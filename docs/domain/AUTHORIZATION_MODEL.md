...
Bootstrapping platform-admin authority for an **already-provisioned** CMP `User`: `scripts/manage_platform_admin.py grant --oidc-issuer ... --oidc-subject ...` — a controlled administrative CLI, not an HTTP route (there is deliberately no unauthenticated "make me a platform admin" endpoint). `grant`/`revoke` require an existing `User` (real OIDC issuer+subject) and never create one.

Bootstrapping the very first platform admin on a **blank** production database (DEPLOY-001A, resolving the chicken-and-egg gap PILOT-SETUP-001B2 originally deferred): `scripts/manage_platform_admin.py bootstrap-first-admin --oidc-issuer ... --oidc-subject ... --email ... --display-name ...`. This is the one command that may create a `User` — `app.services.platform_admin_service.bootstrap_first_platform_admin` resolves-or-creates it by exact `(oidc_issuer, oidc_subject)` identity and atomically grants platform-admin authority (both succeed together or neither survives, via the same outer-transaction/savepoint pattern `platform_tenant_service.onboard_tenant` established). Like `grant`/`revoke`, it requires direct `DATABASE_URL` access, never touches `TenantContext`, and never creates a `Tenant`, `TenantMembership`, or password/local credential — see `docs/deployment/PILOT_DEPLOYMENT.md` for the end-to-end pilot bootstrap procedure.

**Not yet built:** any HTTP route gated by `require_platform_admin` beyond `/platform/tenants` (below), and any web UI for granting/revoking platform authority.

## Platform Tenant onboarding (PILOT-SETUP-001B2)

Builds the first `require_platform_admin`-gated HTTP routes on top of the B1 primitive above: `GET /platform/tenants`, `GET /platform/tenants/{tenant_id}`, `POST /platform/tenants` (`app.api.platform_tenants`, mounted unconditionally in `app.main.create_app` — never conditioned on `ENABLE_DEV_AUTH`/environment, since authorization is decided per-request by `require_platform_admin`, not by whether the route exists). Every route depends on `require_platform_admin` and nothing else — no `require_tenant_context`, no `require_permission`, no `X-CMP-Tenant-Id`/`X-Dev-Tenant-Id` header of any kind (proof: `tests/test_platform_tenants_architecture.py`). These routes expose Tenant metadata only (id/code/name/status via the existing `TenantRead`) — never Farm/Location/CropBatch/Harvest/post-harvest or any other tenant-operational data.

**`POST /platform/tenants` is one onboarding command, not three separate writes:** it creates a Tenant, resolves-or-creates its initial administrative User by exact `(oidc_issuer, oidc_subject)` identity (the same identity-binding rule "Identity binding" above already establishes — no email-based matching, ever), and establishes an active `tenant_admin` `TenantMembership` linking them. `app.services.platform_tenant_service.onboard_tenant` owns this atomically:

- **Identity provisioning is administrative, not authentication.** The Platform Admin is vouching for an OIDC binding they did not themselves prove cryptographically at this moment — CMP stores the `(oidc_issuer, oidc_subject, email, display_name)` the Platform Admin supplied, never a password or any other local credential. Supplying an identity through this API does not log that person in; their first real login still goes through the ordinary bearer-token path (`app.core.oidc`/`app.core.auth`) and must independently present a valid token whose issuer+subject matches. `require_authenticated_principal` is completely unweakened by this ticket.
- **Never silently retargets an existing identity.** If `(oidc_issuer, oidc_subject)` already resolves to a User, that User is reused (never duplicated) — but if the caller's supplied email disagrees with that User's own already-recorded email, the whole command is rejected (`AdminIdentityEmailMismatchError` → `409`) rather than silently overwritten.
- **Transaction strategy:** none of `tenant_service.create_tenant`/`user_service.create_user`/`membership_service.add_membership` was modified — each still commits internally exactly as every other caller (e.g. `app.api.dev_bootstrap`) expects. `onboard_tenant` instead reuses the transaction pattern this codebase already established for the identical problem (`app.services.pilot_bootstrap_service` + `scripts/bootstrap_pilot_master_data.py`): it opens its own `Connection`, begins one outer transaction, and binds an ORM `Session` to it with `join_transaction_mode="create_savepoint"` — every internal `db.commit()` the three services perform then only releases a SAVEPOINT, and `onboard_tenant` alone commits (all three steps succeeded) or rolls back (any step raised) the real outer transaction, once, at the end. A regression test (`tests/test_platform_tenant_service.py::test_membership_step_failure_rolls_back_tenant_and_new_user`) forces the Membership step to fail and proves, from a separate connection, that neither the new Tenant nor the newly-created User survive.
- **`AuditEvent` is unchanged by this ticket** (`AuditEvent.tenant_id` remains `NOT NULL` — no orchestration-level "onboarding" event exists, or could exist, above the Tenant it's about). Tenant creation and Membership creation each still emit their own existing, unmodified tenant-scoped audit event; User creation still follows `user_service.create_user`'s pre-existing, tenant-less precedent (no audit event — the same situation `platform_admin_service.grant_platform_admin` already documents for the identical structural reason).

**Platform Admin gains no implicit Tenant access from calling this.** `onboard_tenant`'s signature carries no "calling actor" parameter at all — only the Tenant fields and the requested `initial_admin` identity — so there is no code path by which the Platform Admin who called `POST /platform/tenants` could themselves receive the resulting `tenant_admin` Membership; only the requested `initial_admin` identity does (test: `tests/test_platform_tenants_http.py::test_platform_admin_caller_receives_no_membership_only_initial_admin_does`). If a Platform Admin wants their own access to a tenant they onboarded, that requires an entirely separate, ordinary `TenantMembership` through the normal tenant-scoped model — unchanged, and out of this ticket's scope.

**Duplicate/conflict handling:** a duplicate Tenant `code` is `409` (existing `DuplicateTenantCodeError`, unchanged) — this alone already prevents the realistic double-apply case (a retried onboarding request for an already-onboarded tenant code never reaches the User/Membership steps at all). A concurrent race creating the same new OIDC identity or the same active Membership is handled by the underlying services' own existing database-level uniqueness (`DuplicateUserIdentityError`/`DuplicateMembershipError`), both mapped to `409`.

**Explicitly out of scope for PILOT-SETUP-001B2** (unchanged/not built): `/admin/tenants` frontend, Farm creation UI, invitations/pending-provisioning/self-service signup, password auth, email verification, OIDC provider management, a platform-admin web grant/revoke UI, new tenant-scoped permissions or role codes, any `AuditEvent` redesign. Farm creation for an onboarded Tenant happens afterward, through the existing, unchanged `POST /farms` flow (its own `tenant_admin`-gated `Permission.FARM_MANAGE` route) — this ticket only gets a Tenant and its first `tenant_admin` to that starting line.
