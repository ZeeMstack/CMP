...
| Condition | Status |
|---|---|
| No/invalid bearer, no dev headers at all | `401` (unchanged — `require_tenant_context`) |
...
| Real bearer: valid identity, but the selected tenant is inactive, or the caller has no active membership for it (including a membership that exists but was removed) | `403` — `require_tenant_context`'s own inline checks (unchanged, pre-dates AUTHZ-001A; see `tests/test_auth_context.py::test_removed_membership_is_403`, `::test_inactive_tenant_is_403`) |
| Dev-auth: the same condition (inactive tenant, or membership missing/inactive) | `401` — `app.core.dev_auth.resolve_dev_tenant_context` (unchanged, pre-dates AUTHZ-001A; see "Known inconsistency" below) |
...
Cross-tenant concealment is never converted into a `403` — a resource in a tenant the caller can't see must remain indistinguishable from a resource that doesn't exist at all, regardless of what the caller is permitted to do in their *own* tenant.

**Known inconsistency (AUTHZ-001A.2 finding; not introduced by AUTHZ-001A, not fixed here):** the real bearer path and the dev-auth path disagree on the status code for "identity is valid but the selected tenant is inactive, or there is no active membership for it" — real bearer returns `403` (the intended contract: authentication succeeded, tenant/membership access did not), dev-auth returns `401`. Both behaviors predate AUTHZ-001A (`git diff 3d3f5ac -- apps/api/app/core/dev_auth.py apps/api/app/core/auth.py` is empty — neither file has been touched by AUTHZ-001A/A.1/A.2). The minimal fix, if approved, would be changing the two `status.HTTP_401_UNAUTHORIZED` raises for "Invalid or inactive tenant context" / "No active membership for this tenant" in `app.core.dev_auth.resolve_dev_tenant_context` to `status.HTTP_403_FORBIDDEN`, matching `require_tenant_context`'s own inline equivalents. Not implemented as part of AUTHZ-001A/A.1/A.2 — it is a dev-auth-only authentication-semantics change, outside this ticket's authorization-foundation scope, and is recorded here for an explicit future decision.
