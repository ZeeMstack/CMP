...
- **Workflow stage / transition** — belong to one workflow version. A stage carries `stage_category` (`seeding`, `germination`, `nursery`, `transplanting`, `intermediate`, `production`, `harvest_ready`, `harvesting`, `completed`, `rejected` — operational categories, not crop names), `is_start`/`is_terminal` flags, and optional `permitted_location_type`/`required_carrier_type` references. A transition connects two stages of the same version; self-transitions are rejected. `required_carrier_type_id` identifies the carrier type a stage-specific operation may newly assign while the batch occupies that stage — e.g. sowing (CMP-009) requires it for carriers a seeding-category stage opens, and transplantation (CMP-011) requires it for carriers a transplanting-category stage opens. `harvesting` (CMP-013) does not require it — harvest opens no carrier assignment, it only reads existing active ones. `harvest_ready` and `harvesting` are distinct: `harvest_ready` may represent operational readiness before harvesting and is not gated on by any command; `harvesting` is the specific category CMP-013's harvest command requires the batch's current stage to carry. It does not retroactively require carriers that were already active before the batch entered the stage to match.
