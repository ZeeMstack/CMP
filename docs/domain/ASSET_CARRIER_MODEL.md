...
**Types.** Global, system-defined, seeded by migration — not tenant-configurable, no type-management API. `asset_types` (`germination_trolley`, `transfer_trolley`, `seeding_machine`, `weighing_scale`, `label_printer`, and `water_quality_meter` since PILOT-WATER-001A) carry a `supports_positions` behavior flag — true only for `germination_trolley` — plus, since PILOT-ASSET-001, `readiness_tracked`/`requires_cleaning` (see below). `carrier_types` (`seed_tray`, `cultivation_plate`, `grow_cube`, `grow_bag`, `harvest_crate`, and `nursery_cultivation_plate`/`production_cultivation_plate` since CARRIER-CONFIG-001B) carry the equivalent `readiness_tracked`/`requires_cleaning` pair. `grow_table`/`grow_gutter` asset records and finished-goods/pallet carrier types are deferred to a later ticket.
...
**Equipment Readiness (PILOT-ASSET-001, implemented).** The "maintenance condition is a separate, future concern" noted above is now built: `EquipmentReadinessState` answers WHETHER a readiness-tracked Asset/Carrier is currently usable (clean/ready, awaiting cleaning, damaged, under maintenance, or terminally retired), kept as its own current-state row, deliberately never merged into this `status` column. A `retire` readiness command does move this `status` to `retired` (the two "retired" facts are set together, atomically) — the only point where the two models touch. See `docs/domain/EQUIPMENT_READINESS_MODEL.md`.
...
**Deferred:** occupancy, movement, farm-location assignment, crop batches/contents/quantities, transformations, label printing, maintenance work orders, and inventory valuation. Permanent QR/scan identity is no longer deferred — see "Permanent QR identity (PILOT-SCAN-001D)" below.

## Permanent QR identity (PILOT-SCAN-001D)

Every Carrier and every Asset automatically receives its own permanent, active `QrIdentifier` row at creation — through `register_carrier`, `bulk_register_carriers`, and `register_asset`, and through Farm Setup's own composite creation of Trolleys/Levels-eligible Assets and structural Carriers. The QR identifies the physical Carrier/Asset record itself and never changes because its current Batch occupant, location, or status changes; reusing a Carrier for a different Batch never changes that Carrier's own QR token. Batch↔Carrier occupancy remains entirely a `BatchCarrierAssignment` concern (see `TRANSPLANTATION_MODEL.md`/`OCCUPANCY_MOVEMENT_MODEL.md`), never encoded into the Carrier's or Asset's own permanent identity. Full lifecycle, scan-resolution behavior (including why a `batch_carrier_assignment` QR — not the Carrier's own QR — is used for stage/placement labels), the pre-existing-master-data backfill, and the frozen "Permanent Physical Identity"/"Historical Assignment" terms are in `QR_SCAN_MODEL.md`.
