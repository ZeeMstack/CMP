...
> **POSTHARVEST-OPS-001F update**: `GradedProduceLot` becomes a first-class public traceability entity — POSTHARVEST-OPS-001E already widened the *internal* lineage join (`FinishedGoodsLot -> PackingEvent -> PackingInputLine -> GradedProduceLot -> GradingEvent -> HarvestedProduceLot -> CropBatch`) but left the GPL hop implicit behind `harvested_produce_lot_id` in every public response; this ticket exposes it directly, on the same three existing GET routes — no new endpoint, no route proliferation. `PackingInputLineRead` gains `graded_produce_lot_id` alongside its existing (unchanged, still-derived) `harvested_produce_lot_id`. The backward trace (`.../finished-goods-lots/{id}`) gains `graded_produce_lots` (every source GPL, immutable identity/quantity/timestamp fields only) and `grading_events` (every distinct `GradingEvent` that produced one of them) — both empty-checked against `packing_inputs`/`graded_produce_lots` respectively and raising `TraceabilityIntegrityError` on an unresolved FK, exactly like the pre-existing `produce_lots` check. Sibling-grade isolation is structural: a `GradingEvent` may parent several distinct `GradedProduceLot`s (e.g. Grade A and Grade B), but only the GPL(s) actually referenced by this FG lot's own packing input lines are ever surfaced — never a sibling output of the same event. Both forward-impact endpoints (`crop-batches/{id}/impact`, `harvested-produce-lots/{id}/impact`) gain `graded_produce_lots`, each row carrying `is_affected_source` (true iff its one source HPL is affected — a GPL is never partially affected, since it has exactly one source HPL via its `GradingEvent`), mirroring the existing `packing_inputs[].is_affected_source` co-input-context rule one lineage hop further out; `ImpactSummary` gains `affected_graded_produce_lot_count`. No proportional attribution is introduced. No migration — the relational lineage already existed after 001E; this is a read-model/schema-only extension.
