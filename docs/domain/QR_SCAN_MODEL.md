...
- **Operational Placement Label**: identifies ONE specific physical carrier/placement (Seed Tray, Nursery/Production Cultivation Plate, Grow Cube, Grow Bag) with a print-time snapshot of the Batch/stage/location context it currently holds. Its QR is the **same Carrier QR identity** (`entity_type = "carrier"`) as PILOT-SCAN-001's own Permanent Carrier Label — only the printed TEXT differs (stage-aware vs. bare identity). Every supported stage (including Germination, whose own command response carries no `batch_carrier_assignment_id`) uses this one uniform entity type, deliberately, so no backend/API change was needed and "Reprint" for any of these later can reuse the exact same generic Carrier label route PILOT-SCAN-001 already built (`/farms/[farmId]/labels/carrier/[carrierId]`), which re-resolves current state rather than replaying a stale snapshot.

`lib/labels/operationalLabel.ts` is the one small `OperationalLabelContext` builder module (`{ size, entityTypeLabel, code, lines }`) — presentation composition only, never a new source of domain truth: every field a builder function accepts must already come from the triggering command's own just-returned result. One function per stage: `batchMasterLabel`, `sowingTrayLabel`, `germinationPlacementLabel`, `seedlingEntryLabel`, `intersaladsPlacementLabel`, `leafyProductionPlacementLabel`, `intervinesPlacementLabel`, `vinesProductionPlacementLabel`. `lib/labels/usePreparedPrintLabels.ts` resolves each spec's QR token (idempotent `generate_or_get_qr_identifier`, unchanged from PILOT-SCAN-001) in the background as soon as a success screen renders, producing the `PrintableLabel[]` a "Print Labels" click hands to `openLabelPrintWindow`.
