...
**Status: IMPERIAL PILOT POLICY ACTIVATED IN AUTHZ-002B2.** Matrix A (§12) below is now live in `app.core.permissions.ROLE_PERMISSIONS` — every one of the 12 approved roles has the exact grant set this document specifies for the Imperial Pilot, mechanically verified against this matrix by `tests/test_permissions.py`. `farm_manager` was activated at the **minimum viable tier (25 permissions, §3.A)** — the "broader pilot" `dispatch.manage` cell (marked `G*` in Matrix A) was deliberately **not** activated; that remains a documented, available-but-inactive option. Matrix B (External Commercial V1) remains a **design proposal only** — nothing in Matrix B beyond what Matrix A already specifies is active, and this document's §1/§13 P1/P2 gap list (farm-scoped role assignment, quality-hold place/release split, recall open/close split, no general Input/Store module, no `audit.read`, and the rest) is **unaffected by this activation** and remains open, tracked hardening work for a future, explicitly-scoped ticket. This document continues to be the design/policy record; the implementation itself lives in `app/core/permissions.py` and its tests.
...
**Revision note (AUTHZ-002B2) — Matrix A is now ACTIVE.** `ROLE_PERMISSIONS` was updated to grant every non-`tenant_admin` role exactly the set Matrix A specifies (farm_manager at the minimum tier, §3.A — the `G*` broader-pilot `dispatch.manage` cell was deliberately left inactive). This resolves priority gap #2 in §13 operationally, not just at the permission-model level (which AUTHZ-002B1 already resolved). Every other open finding in this document — the farm-scope architecture gap (§1), the quality-hold and recall bundling (§6/§10), the missing Input/Store module (§7), and the `audit.read` discoverability gap (§9) — is **not** addressed by this activation and remains exactly as described below.
...
## 12. Matrices (Matrix A is ACTIVE as of AUTHZ-002B2; Matrix B remains a proposal only)

Two distinctions matter here, not one: (a) Imperial Pilot vs. External Commercial V1, and (b) **PROPOSED** (this document's recommended policy — every `G` in the matrices below) vs. **ACTIVE** (what `ROLE_PERMISSIONS` actually grants today, in code, right now). As of AUTHZ-002B2, **Matrix A is fully active**: every non-`tenant_admin` role holds exactly the `G` cells below (farm_manager at the minimum tier — its `G*` `dispatch.manage` cell remains proposed-only, not activated). Matrix B (§ below) remains **proposed only** — nothing beyond Matrix A's own grants is active.

### MATRIX A — Imperial Pilot (ACTIVE as of AUTHZ-002B2, except the `G*` cell noted below)
...
`*` = `farm_manager`'s `dispatch.manage` is the "broader pilot" tier grant (§3.B) — **AUTHZ-002B2 deliberately did NOT activate this cell**; the pilot decision was to activate the "minimum viable" tier only (§3.A, FM 25, this cell excluded). Every other `G` cell in this matrix, including `production_supervisor`/`operator`/`qc_officer`'s `observation_entry.manage` cells, is now genuinely active in `ROLE_PERMISSIONS`, not merely proposed.

Grant totals (mechanically verified against this matrix, and against `ROLE_PERMISSIONS` as of AUTHZ-002B2): TA **42 (active)**, FM **25 (active, minimum tier — 26/broader tier remains inactive)**, **HG 25 (active)**, **PS 24 (active)**, **OP 16 (active)**, SK **6 (active)**, **QC 19 (active)**, PK **12 (active)**, CS **11 (active)**, DO **11 (active)**, AU **20 (active)**, RO **20 (active)**.
...
| 2 | `observation.manage` bundled routine entry + definition configuration (§5, §5A) | **FULLY RESOLVED (AUTHZ-002B1 + AUTHZ-002B2)** — `observation_entry.manage`/`observation_definition.manage` exist, are correctly wired, and `ROLE_PERMISSIONS` now grants `observation_entry.manage` to `production_supervisor`/`operator`/`qc_officer` (and `observation_definition.manage` to `head_grower`) exactly per Matrix A | Same grants are active regardless of farm count — this item does not reintroduce a gap at multi-farm scale | Same activation carries over; farm-scoping (item #1) is the separate, still-open question for this deployment shape |
...
**Post-AUTHZ-002B2 status**: the observation-granularity item (#2) — previously the sole disciplined P0 blocking the Imperial pilot — is now fully resolved, both at the permission-model level (AUTHZ-002B1) and operationally (AUTHZ-002B2 activated the grants). `production_supervisor`/`operator`/`qc_officer` can record observations and `head_grower` can define them, today, in production. Item #1 (farm-scoped role assignment) remains correctly excluded from the pilot's own P0 list — P2 for the Imperial single-farm pilot itself, with no practical effect until a second farm or an external multi-farm customer is in scope. Everything else (#3–#8) remains a real, tracked hardening item for external commercialization, entirely unaffected by AUTHZ-002B2's activation — activating Matrix A did not touch the quality-hold/recall bundling, the missing Input/Store module, or the `audit.read` gap.

---

## Summary of what this document does NOT do

**Original scope (AUTHZ-002A), as a design/policy document:**
- Does not itself modify `app.core.permissions.Permission` or `ROLE_PERMISSIONS` — those changes were made by the implementation tickets this document authorized (AUTHZ-002B1 for the permission catalog split, AUTHZ-002B2 for activating Matrix A's grants), not by editing this file.
...
**Still true after AUTHZ-002B2's activation:**
