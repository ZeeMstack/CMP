...
| Operational Placement Label | (PILOT-SCAN-001B) A printed label identifying ONE specific physical carrier/placement (a Seed Tray, Nursery/Production Cultivation Plate, Grow Cube, or Grow Bag) together with a print-time snapshot of the Batch/stage/location context it currently holds. The QR itself is that same Carrier's own Permanent Carrier Label identity (`entity_type = "carrier"`) — only the printed TEXT is stage-aware; the QR payload never encodes the stage/location itself |
| Permanent Carrier Label | (PILOT-SCAN-001) A printed label whose QR identifies a Carrier's own permanent physical identity (`entity_type = "carrier"`), with no stage-specific context — PILOT-SCAN-001's original generic "Print Label" content. An Operational Placement Label reuses this exact same QR identity, adding stage-aware printed text on top |
...
| Reprint | (PILOT-SCAN-001) A second (or later) Print Request against the same already-existing QR identity — never a new QR, a new Batch/Lot/Carrier, or a new operational event. PILOT-SCAN-001B's per-stage "Print Labels" actions are always an initial print request for a freshly created identity; reprinting a Stage Label later is done from the entity's own generic label/reprint route (`/farms/[farmId]/labels/[entityType]/[entityId]`), which re-resolves current authoritative context at reprint time |
