...
| Operational Placement Label | (PILOT-SCAN-001B, entity rule frozen at FINAL CLOSURE) A printed label identifying ONE specific physical carrier/placement (a Seed Tray, Nursery/Production Cultivation Plate, Grow Cube, or Grow Bag) together with a print-time snapshot of the Batch/stage/location context it currently holds. The QR identifies the strongest existing stable Batch-placement identity the stage's own authoritative result/read model provides — `entity_type = "batch_carrier_assignment"` wherever available (every stage except Germination placement, whose own command response carries no assignment id — see `docs/domain/QR_SCAN_MODEL.md`'s per-stage table); a bounded, documented fallback to the Carrier's own Permanent Carrier Label identity (`entity_type = "carrier"`) applies only there, and only when no matching placement id can be found. The QR payload never encodes the stage/location itself — only the printed TEXT is stage-aware |
